...
def processar_temperaturas(path_do_txt: Path):
    """
    Processa um arquivo de temperaturas e calcula as estatísticas mínimas, médias e 
    máximas para cada estação.

    Parâmetros:
        path_do_txt (Path): O caminho para o arquivo de texto que contém os dados 
        de temperatura.

    Retorna:
        dict: Um dicionário contendo as estatísticas formatadas para cada estação.
    """
    print("Iniciando o processamento do arquivo.")
    start_time = time.time()  # Tempo de início
    temperatura_por_station = defaultdict(list)

    # Exemplo de como a variável temperatura_por_station será preenchida:
...
if __name__ == "__main__":
    ...
    for station, stats in resultados.items():
